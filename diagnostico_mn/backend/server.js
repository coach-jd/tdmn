...
app.use(cors({
    origin: ['http://localhost:5500', 'http://127.0.0.1:5500', 'https://waymentorlatam.com'],  // Permite los orígenes locales y de producción
...
app.post('/send-email', async (req, res) => {
  ...
  if (!pdfBase64 || !name || !email) {
    return res.status(400).json({ message: 'Faltan datos requeridos: nombre, correo o PDF.' });
  }

  // Lee la plantilla del correo
  const emailTemplate = fs.readFileSync(path.join(__dirname, 'templates', 'emailTemplate.html'), 'utf8');
  ...
  const msg = {
    ...
    bcc: 'diagnostico_mn@waymentorlatam.com', // Solo una copia oculta a este correo
  ...
  try {
    ...
    console.error('Error al enviar el correo:', error.response ? error.response.body.errors : error.message);
    res.status(500).json({ message: 'Hubo un error al enviar el correo', error: error.response ? error.response.body : error.message });
  }
});
